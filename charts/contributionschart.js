(function d3ChartIIFE() {
	...
	const svgWidth = 140,
		...
		helpPortalUrl =
			"https://gms.unocha.org/content/business-intelligence#CBPF_Contributions",
		dataUrl =
	...
	chartState.selectedOrder =
		queryStringValues.has("order") &&
		orders.includes(queryStringValues.get("order"))
			? queryStringValues.get("order")
			: orders.includes(containerDiv.node().getAttribute("data-order"))
			? containerDiv.node().getAttribute("data-order")
			: orders[0];

	chartState.selectedDonors =
		queryStringValues.has("donors") &&
		donors.includes(queryStringValues.get("donors"))
			? queryStringValues.get("donors")
			: donors.includes(containerDiv.node().getAttribute("data-donors"))
			? containerDiv.node().getAttribute("data-donors")
			: donors[0];
	...
	const headerDiv = containerDiv
		.append("div")
		.attr("class", classPrefix + "headerDiv")
		.style(
			"height",
			containerDiv.node().offsetWidth * headerDivHeightPercentage + "px"
		);

	const topFiguresDiv = headerDiv
		.append("div")
		.attr("class", classPrefix + "topFiguresDiv");

	const topFiguresHeaderDiv = topFiguresDiv
		.append("div")
		.attr("class", classPrefix + "topFiguresHeaderDiv");

	const topFiguresDonorsDiv = topFiguresDiv
		.append("div")
		.attr("class", classPrefix + "topFiguresDonorsDiv");

	const topFiguresDonorsNumberDiv = topFiguresDonorsDiv
		.append("div")
		.attr("class", classPrefix + "topFiguresDonorsNumberDiv");

	const topFiguresDonorsTextDiv = topFiguresDonorsDiv
		.append("div")
		.attr("class", classPrefix + "topFiguresDonorsTextDiv");

	const topFiguresContributionsDiv = topFiguresDiv
		.append("div")
		.attr("class", classPrefix + "topFiguresContributionsDiv");

	const topFiguresContributionsNumberDiv = topFiguresContributionsDiv
		.append("div")
		.attr("class", classPrefix + "topFiguresContributionsNumberDiv");

	const topFiguresContributionsTextDiv = topFiguresContributionsDiv
		.append("div")
		.attr("class", classPrefix + "topFiguresContributionsTextDiv");

	const topChartDiv = headerDiv
		.append("div")
		.attr("class", classPrefix + "topChartDiv");

	const topChartHeaderDiv = topChartDiv
		.append("div")
		.attr("class", classPrefix + "topChartHeaderDiv");

	const topChartSvgDiv = topChartDiv
		.append("div")
		.attr("class", classPrefix + "topChartSvgDiv");

	const topChartSvg = topChartSvgDiv
		.append("svg")
		.attr("viewBox", "0 0 " + topSvgWidth + " " + ~~topSvgHeight)
		.attr("preserveAspectRatio", "xMidYMid meet")
		.style("height", "100%");

	const topRadioButtonsDiv = headerDiv
		.append("div")
		.attr("class", classPrefix + "topRadioButtonsDiv");

	const topRadioButtonsDonorsDiv = topRadioButtonsDiv
		.append("div")
		.attr("class", classPrefix + "topRadioButtonsDonorsDiv");

	const topRadioButtonsOrderDiv = topRadioButtonsDiv
		.append("div")
		.attr("class", classPrefix + "topRadioButtonsOrderDiv");

	const chartsDiv = containerDiv
		.append("div")
	...
	const xAxisTopChart = d3
		.axisBottom(xScaleTopChart)
		.tickSizeOuter(0)
		.tickSizeInner(3)
		.tickPadding(2);

	const xAxisGroupTopChart = topChartSvg
		.append("g")
		.attr("class", classPrefix + "xAxisGroupTopChart")
		.attr(
			"transform",
			"translate(0," + (topSvgHeight - topSvgPadding[2]) + ")"
		);
	...
	Promise.all([
		fetchFile("contributionsdata", dataUrl, "data", "csv"),
	...
	function draw(rawData, flagsData) {
		...
		xAxisTopChart.tickValues(
			yearsArrayWithNull.filter(
				(e, i) => e && (!(i % 2) || e === currentYear)
			)
		);
	...
	function createTopFigures(data) {
		let headerText = topFiguresHeaderDiv
			.selectAll("." + classPrefix + "headerText")
			.data([true]);
		...
		headerText = headerText
			...
			.html(
				"Since " +
					yearsArray[0] +
					"," +
					(chartState.selectedDonors === donors[0] ? " the" : "")
			);

		let donorsNumber = topFiguresDonorsNumberDiv
			.selectAll("." + classPrefix + "donorsNumber")
			.data([true]);
		...
		donorsNumber = donorsNumber
			...
			.attr(
				"class",
				classPrefix + "donorsNumber contributionColorHTMLcolor"
			)
			.merge(donorsNumber)
			.html(
				chartState.selectedDonors === donors[0]
					? "Top " + topDonorsNumber
					: data.byDonor.length
			);

		let donorsText = topFiguresDonorsTextDiv
			.selectAll("." + classPrefix + "donorsText")
			.data([true]);
		...
		donorsText = donorsText
			...
			.merge(donorsText)
			.html(
				(data.byDonor.length > 1 ? "Donors" : "Donor") +
					"<br>Contributed"
			);

		const previousValue =
			d3.select("." + classPrefix + "contributionsNumber").size() !== 0
				? d3.select("." + classPrefix + "contributionsNumber").datum()
				: 0;

		let contributionsNumber = topFiguresContributionsNumberDiv
			.selectAll("." + classPrefix + "contributionsNumber")
			.data([
				d3.sum(
					chartState.selectedDonors === donors[0]
						? data.topDonors
		...
		contributionsNumber = contributionsNumber
			...
			.attr(
				"class",
				classPrefix + "contributionsNumber contributionColorHTMLcolor"
			)
		...
		let contributionsText = topFiguresContributionsTextDiv
			.selectAll("." + classPrefix + "contributionsText")
			.data([
				d3.sum(
					chartState.selectedDonors === donors[0]
						? data.topDonors
	...
	function createTopChart(data) {
		const topChartHeaderText = `donors contributions to CERF, ${
			yearsArray[0]
		} to ${yearsArray[yearsArray.length - 1]} (hatched), in US$`;

		let topChartHeader = topChartHeaderDiv
			.selectAll("." + classPrefix + "topChartHeader")
			.data([true]);

		topChartHeader = topChartHeader
			.enter()
			.append("span")
			...
			.html(
				(chartState.selectedDonors === donors[0]
					...
					(alreadyHovered
						? ""
						: "<br>(hover over the bars for highlighting a year)")
			);
		...
		xAxisGroupTopChart.transition(syncedTransition).call(xAxisTopChart);

		let barsTop = topChartSvg
			.selectAll("." + classPrefix + "barsTop")
		...
		let barLineTop = topChartSvg
			.selectAll("." + classPrefix + "barLineTop")
			.data(
				fillWithZeros(topChartData.filter(e => e.year < currentYear))
			);
		...
		barLineTop.transition(syncedTransition).attr("d", topLineGenerator);

		let lastYearCircleTop = topChartSvg
			.selectAll("." + classPrefix + "lastYearCircleTop")
			.data([
				topChartData.find(e => e.year === currentYear - 1) ||
					zeroObject,
			]);

		lastYearCircleTop = lastYearCircleTop
			.enter()
			.append("circle")
		...
		let lastYearLineTop = topChartSvg
			.selectAll("." + classPrefix + "lastYearLineTop")
			.data([
				topChartData.find(e => e.year === currentYear - 1) ||
					zeroObject,
			]);

		lastYearLineTop = lastYearLineTop
			.enter()
			.append("polyline")
			.attr("class", classPrefix + "lastYearLineTop")
			.attr("points", (d, i, n) => {
				return `${
					xScaleTopChart(currentYear - 1) +
					lastYearCircleTopRadius +
					bandwidth / 2
				},${yScaleTopChart(0)} 
				${
					xScaleTopChart(currentYear - 1) +
		...
		lastYearLineTop
			...
			.attr("points", (d, i, n) => {
				...
				${
					...
					topSvgHeight - topSvgPadding[2] - labelMinPadding,
					yScaleTopChart(d.amount)
				)} 
				${xScaleTopChart(currentYear - 1) + barLabelTopPadding},${Math.min(
					topSvgHeight - topSvgPadding[2] - labelMinPadding,
					yScaleTopChart(d.amount)
				)}`;
			});

		let barLabelTop = topChartSvg
			.selectAll("." + classPrefix + "barLabelTop")
			.data([
				topChartData.find(e => e.year === currentYear - 1) ||
					zeroObject,
			]);

		barLabelTop = barLabelTop
			.enter()
			.append("text")
		...
		let allLabelsTop = topChartSvg
			.selectAll("." + classPrefix + "allLabelsTop")
			.data(topChartData);

		allLabelsTop = allLabelsTop
			.enter()
			.append("text")
			...
			.attr("y", d => yScaleTopChart(d.amount))
			.text(d =>
				d3.formatPrefix(".0", d.amount)(d.amount).replace("G", "B")
			);

		const barsTopOver = topChartSvg
			.selectAll("." + classPrefix + "barsTopOver")
			.data(yearsArray)
			.enter()
			.append("rect")
			...
			.attr(
				...
				d =>
					xScaleTopChart(d) -
					(xScaleTopChart.step() - xScaleTopChart.bandwidth()) / 2
			)
			.attr("width", xScaleTopChart.step())
			.attr("y", yScaleTopChart.range()[1])
			.attr(
				"height",
				yScaleTopChart.range()[0] - yScaleTopChart.range()[1]
			);
		...
		barsTopOver
			.on("mouseover", year => {
				if (!alreadyHovered) {
					alreadyHovered = true;
					topChartHeaderDiv
						.select("." + classPrefix + "topChartHeader")
						.html(
							(chartState.selectedDonors === donors[0]
								? `Top ${topDonorsNumber} `
								: "All ") +
				...
				barsTop.style("opacity", d =>
					d.year === year ? 1 : fadeOpacity
				);
				highlightSelection.donorSvg
					.select("." + classPrefix + "xAxisGroup")
					.call(xAxis);
				highlightSelection.donorSvg
					.select("." + classPrefix + "barLine")
					.style("opacity", 0);
				highlightSelection.donorSvg
					.select("." + classPrefix + "lastYearCircle")
					.style("opacity", 0);
				highlightSelection.donorSvg
					.select("." + classPrefix + "lastYearLine")
					.style("opacity", 0);
				highlightSelection.donorSvg
					.select("." + classPrefix + "barLabel")
					.style("opacity", 0);
				highlightSelection.donorSvg
					.selectAll("." + classPrefix + "allLabels")
					.style("opacity", d => (d.year === year ? 1 : 0));
				highlightSelection.donorSvg
					.selectAll("." + classPrefix + "bars")
					.style("opacity", d => (d.year === year ? 1 : fadeOpacity));
			})
			.on("mouseout", () => {
				xAxisTopChart.tickValues(
					yearsArrayWithNull.filter(
	...
	function createTopRadioButtons(data) {
		const donorLabels = topRadioButtonsDonorsDiv
			.selectAll(null)
			.data(donors)
			.enter()
			.append("label")
		...
		const donorInputOuterSpan = donorLabels
			.append("span")
			.attr("class", classPrefix + "radioInput");

		const donorInput = donorInputOuterSpan
			.append("input")
			.attr("type", "radio")
		...
		const donorInputInnerSpan = donorInputOuterSpan
			.append("span")
			.attr("class", classPrefix + "radioControl");

		const donorsText = donorLabels
			.append("span")
			.attr("class", classPrefix + "radioLabel")
			.html(d =>
				d === donors[0] ? `Top ${topDonorsNumber} Donors` : "All donors"
			);

		const orderLabels = topRadioButtonsOrderDiv
			.selectAll(null)
			.data(orders)
			.enter()
			.append("label")
		...
		const orderInputOuterSpan = orderLabels
			.append("span")
			.attr("class", classPrefix + "radioInput");

		const orderInput = orderInputOuterSpan
			.append("input")
			.attr("type", "radio")
		...
		const orderInputInnerSpan = orderInputOuterSpan
			.append("span")
			.attr("class", classPrefix + "radioControl");

		const ordersText = orderLabels
			.append("span")
			.attr("class", classPrefix + "radioLabel")
			.html(d =>
				d === orders[0] ? "By contributions" : "By alphabetical order"
			);
	...
	function createChartDivs(dataByDonor, flagsData) {
		dataByDonor.sort((a, b) =>
			chartState.selectedOrder === orders[0]
				? b.total - a.total
		...
		const donorDiv = chartsDiv
			...
			.style("display", d =>
				chartState.selectedDonors === donors[0] &&
				!topDonorsIsoCodes.includes(d.isoCode)
					? "none"
					: null
			)
				...
				d3
					...
					.domain([
						...
						d3.max(d.contributions, e =>
							d3.max(d.contributions, e => e.amount)
						),
					])
			);
		...
		const bars = donorSvg
			...
			.attr(
				"height",
				(d, i, n) =>
					localyScale.get(n[i])(0) - localyScale.get(n[i])(d.amount)
			);

		const barLine = donorSvg
			.selectAll(null)
			.data(d =>
				fillWithZeros(
		...
		const lastYearLine = donorSvg
			...
			.attr("points", (d, i, n) => {
				...
				return `${
					xScale(currentYear - 1) + bandwidth / 2
				},${thisLocalScale(d.amount)} 
				${xScale(currentYear - 1) + bandwidth / 2},${Math.min(
	...
	function hideDonors() {
		highlightSelection.donorDiv.style("display", d =>
			chartState.selectedDonors === donors[0] &&
			!topDonorsIsoCodes.includes(d.isoCode)
				? "none"
				: null
	...
	function sortDonors(dataByDonor) {
		highlightSelection.donorDiv.sort((a, b) =>
			chartState.selectedOrder === orders[0]
				? b.total - a.total
				: a.donor.localeCompare(b.donor)
		);
	...
	function processData(rawData) {
		...
		rawData.forEach(row => {
			if (row.FiscalYear <= currentYear) {
				...
				const foundDonor = data.byDonor.find(
					e =>
						e.isoCode ===
						(row.GMSDonorISO2Code
							? row.GMSDonorISO2Code.toLowerCase()
							: row.GMSDonorISO2Code)
				);
				...
				const foundAllDonorsYears = data.allDonors.find(
					e => e.year === row.FiscalYear
				);
		...
		yearsArray.forEach(year =>
			data.topDonors.push({ year: year, amount: 0 })
		);

		data.byDonor.reduce((acc, curr, index) => {
			if (index < topDonorsNumber) {
				topDonorsIsoCodes.push(curr.isoCode);
				curr.contributions.forEach(
					contrib =>
						(acc.find(e => e.year === contrib.year).amount +=
							contrib.amount)
				);
			}
			return acc;
	...
	function createFooterDiv() {
		...
		const footerLink =
			" | For more information, please visit <a href='https://pfbi.unocha.org'>pfbi.unocha.org</a>";
	...
	function createSnapshot(type, fromContextMenu) {
		...
		topChartSvg
			.attr("width", topSvgRealSize.width)
		...
		topRadioButtonsDiv
			...
			.style("height", "1.1em");
		topRadioButtonsDiv
			.selectAll("." + classPrefix + "radioControl")
			.style("display", "none");
		...
		html2canvas(imageDiv).then(function (canvas) {
			topChartSvg.attr("width", null).attr("height", null);
